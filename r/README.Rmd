...
You can get the join more concisely with `interpolate_intensive()` as follows:

```{r}
sources_no_na <- source |>
  filter(!is.na(value))
matches <- anime::anime(
  sources_no_na,
  target,
...
class(matches)
interpolated_value = interpolate_intensive(sources_no_na$value, matches)
