...
Identifying correspondence between linestring geometries in the absense of a shared unique identifier continues to challenge spatial data science.
Existing methods tend to focus on binary correspondence between linestrings utilizing buffer or distance rules. 
However, the extent to which two linestrings correspond can be udnerstood as a continuous variable that measures their overlap rather than relying on the assumption of equality between linestrings.
...
The resultant measure of shared lengths ($SL_{ij}$) between features enable extensive and intensive attribute interpolation methods for accurate attribute transfer.
The ANIME algorithm is implemented in `anime`, a Rust crate with bindings to the R and Python programming languages.
A case study on Scottish transport networks demonstrates matching generalized and detailed geometries including attribute integration achieving `r sprintf("%.1f%%", abstract_f1 * 100)` F1-score accuracy in validation experiments.
...
- Correspondence between linestrings is decomposed into component line-segments where candidate mathes are identified by spatial index.
...
- High-performance open-source implementation in Rust with comprehensive R and Python language bindings for the geospatial research community.
...
Many existing methods utilize geometric similarity measures such as proximity, orientation, length ratios, and shape metrics including Fréchet and Hausdorff distances to detmine whether a source and target features correspond [@samal_feature_2004].

<!-- 
  we should probably call out the fact that most conflatioon tools are not open source here as a limitation
  the leading tool is the Esri conflation suite. see https://doc.esri.com/en/arcgis-pro/latest/tool-reference/editing/transfer-attributes.html?tabs=dialog#usage for the closest match
 -->
...
While pioneering, these approaches yield binary matching decisions and struggle with complex topological relationships.
...
<!-- these methods are extremely slow, right? Should we call this out? should there be anything here talking about their limitations -->
...
| Method                                    | Year | Correspondence | Open Source | Attribute Transfer |
|-------------------------------------------|------|---------------------|-------------|------------|
| Buffer Method [@goodchild_simple_1997]    | 1997 | 1:1 (binary) | N/A | No |
| Feature-based [@samal_feature_2004]       | 2004 | 1:1 (binary) | No | No |
| Delimited Strokes [@zhang_methods_nodate] | 2006 | 1:1 (binary) | No | No |
...
| **ANIME (This work)**                     | **2025** | **M:N (shared length)** | **Yes** | **Yes** |

: Comparison of network conflation methods. ANIME is distinguished by its M:N correspondence type with quantitative shared length output and open-source availability. "No" indicates the feature is not natively supported; "Limited" indicates partial or post-hoc support. {#tbl-comparison}


As shown in @tbl-comparison, ANIME addresses key limitations of existing methods by providing: 

1. native support for complex m:n relationships through segment-level decomposition,
2. quantitative correspondence measurement via shared length calculations rather than binary decisions, 
3. open-source implementation ensuring reproducibility and community extensibility, and
4. mathematically grounded attribute transfer methods integrated directly into the conflation workflow.
...
Spatial indexing structures, notably the R-tree [@guttman_rtrees_1984] and variants such as the R*-tree [@beckmann_rstartree_1990], are used to efficiently query candidate matches by identifying overlapping axis-aligned bounding boxes (AABB).
...
The computational constraints are increasingly salient because road networks are now assembled from heterogeneous sources, ranging from authoritative mapping agencies to volunteered geographic information (VGI) platforms.
OpenStreetMap (OSM) provides global coverage of road networks with rapid community updates.
However, its quality varies across space and context.
Early evaluations found urban OSM roads to be within 6 meters of UK Ordnance Survey data on average, while completeness and local accuracy were more variable [@haklay_good_2010].
...
<!--
  The fundamental argument here is that we do't view correspondence at the LineString level, rather,
  we acknowledge that linestrings are composed of smaller lines (2 vertices) that can be compared directly
-->
...
The ANIME algorithm fundamentally rethinks correspondence between source and target linestring geometries.
...
The ANIME matching algorithm produces a match table containing:
...
This design intentionally separates the core geometric matching from subsequent attribute interpolation applications.
...
![Illustration of segment overlap calculation. The shared length is determined by projecting line segments onto either the x-axis (for slopes ≤45°) or y-axis (for slopes >45°) and calculating the intersection of projected ranges.](assets/line-seg-overlap-top.png){#fig-segment-overlap}
:::
...
<!-- Parallel Line Attribute Integration (flow) -->
<!-- Suggestion on this (from Robin): Show the outputs of intensive and extensive attribute transfer for flow data in the case study/Appendix and then mention how the needs of flow estimation differ (due to the need to keep the total flow, length of network / flow per unit length equivalent between source and target networks). We could frame this as a future research problem that requires additional constraints and is not the focus of this paper, but we can mention it as a future direction. -->
...
<!-- add geoarrow mention citation here
we may want to mention that calculations are done using the geo crate with citation as well here or later on
 -->
...
ANIME follows a multi-step architecture with a high-performance implementation written in the Rust programming langauge complemented by language-specific bindings that provide attribute interpolation utilities for broad accessibility across the geospatial community.

### Core components

**Core Algorithm**: The high-performance engine is implemented in Rust. Key components include:
...
<!-- TODO mention parallelized computation via rayon? -->
...
<!-- is this too rust specific for a journal?? -->
...
<!-- what are we trying to get at here? this feels like it was written by claude. 
...
<!-- there isn't an "interpolation system" feels written by Claude. That stresses me out. -->

### Implementation architecture

<!-- so now it is an engine?  -->
...
- **Input**: Source attribute vector and computed match table with shared lengths
...
- **Algorithmic Strategies**:
    ...
    <!-- claude had to have written this. this has nothing to do with the paper. -->
...
<!-- this is probably also entirely unecessary -->
...
<!-- i still don't know what go dutch is -->
